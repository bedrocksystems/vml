...
private:
    ...
    public:
         ...
         */
        bool is_injected(uint8 sender_id = 0) const { return _info & (INJECTED_BIT << sender_id); }
    ...
    struct Irq {
        enum State { INACTIVE = 0, PENDING = 1, ACTIVE = 2, ACTIVE_PENDING = 3 };
...
public:
    ...
    public:
        ...
        Lr(Irq::State const state, Irq &irq, uint32 const vintid, uint8 sender = 0) : _lr(0) {
        ...
        Irq::State state() const { return Irq::State((_lr >> STATE_SHIFT) & STATE_MASK); }
